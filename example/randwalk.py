...
from stream import repeatcall, apply, fold, takei, takewhile, drop, item, reduce
...
get_len = reduce(lambda a,b: a+1, 0)
...
def returned(n):
    ...
    for pos in randwalk() >> drop(1) >> takei(range(n-1)):
        if pos == Origin:
            return True
    return False
...
def first_return():
    ...
    a *very* long time.
    """
    return randwalk() >> drop(1) >> takewhile(lambda v: v != Origin) >> get_len
